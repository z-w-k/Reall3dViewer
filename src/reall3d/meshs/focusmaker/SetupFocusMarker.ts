...
export function setupFocusMarker(events: Events) {
    ...
    on(FocusMarkerUpdateScale, () => {
        const newScale = (fire(GetCameraPosition) as Vector3).distanceTo(css3dFocusMarker.position) * 0.004;
    ...
    on(FocusMarkerUpdate, (focusPosition: Vector3) => {
        ...
        isMobile && css3dFocusMarker.position.multiplyScalar(0.993); // 调整细微偏差
    ...
    function initFocusMarker() {
        ...
        // css3dTag.element.style.transformStyle = 'preserve-3d';
