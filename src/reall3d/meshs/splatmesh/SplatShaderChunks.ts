...
export function defineSplatShaderChunks() {
    shaderChunk['splat'] = `
        ...
        #include <splatReader>
        #include <splatEvalSH>
        #include <splatWatermarkEffect>
        #include <splatFvAlpha>
        #include <splatMarkPoint>
        #include <splatProgressive>
        #include <splatProcessWithCov3d>
        #include <splatChecks>
    `;

    // https://github.com/graphdeco-inria/gaussian-splatting/blob/main/utils/sh_utils.py
    shaderChunk['splatEvalSH'] = `
        uniform highp usampler2D splatShTexture12, splatShTexture3;
        ...
        vec3[15] readShDatas(in uint splatIndex) {
            int shCnt = 0;
            float[45] fSHs;
        ...
        vec3 splatEvalSH(in vec3 v3Cen, in uint splatIndex) {
            if (shDegree <= 0) {
                return vec3(0., 0., 0.);
            }
            ...
            vec3[15] sh = readShDatas(splatIndex);
    ...
    shaderChunk['splatFvAlpha'] = `
        uniform uint flagValue;
        uniform float performanceNow, performanceAct;
        float computeFvAlpha(in uvec4 cen) {
            ...
            if (fvSplat > 0u) {
                ...
                if ( fvSplat == fvShow ) {
                    alpha = clamp((performanceNow - performanceAct) / 2000.0, 0.0, 1.0);
                } else if ( fvSplat == fvHide) {
                    alpha = 1.0 - clamp((performanceNow - performanceAct) / 2000.0, 0.0, 1.0);
                } else {
                    alpha = 0.;
                }
            }
        ...
        bool checkFvVisible(in uvec4 cen) {
            float fvAlpha = computeFvAlpha(cen);
            if ( fvAlpha <= 0.0 ) {
        ...
        void processWatermarkAnimate(in uvec4 cen, out vec3 v3Cen) {
            if (isWatermarkAnimate(cen)) {
                v3Cen.y += sin(performanceNow*0.002 + v3Cen.x) * 0.1; // 水印动画
            }
        }
    `;

    shaderChunk['splatReader'] = `
        uniform highp usampler2D splatTexture0, splatTexture1;
        uniform int usingIndex;
        uniform bool bigSceneMode;

        bool isBigSceneMode() {
            return bigSceneMode;
        }

        uvec4 splatReadCenter(in uint splatIdx) {
            uvec4 cen;
            if (bigSceneMode) {
                if (usingIndex == 0){
                    cen = texelFetch(splatTexture0, ivec2((splatIdx & 0x3ffu) << 1, splatIdx >> 10), 0);
                }else{
                    cen = texelFetch(splatTexture1, ivec2((splatIdx & 0x3ffu) << 1, splatIdx >> 10), 0);
                }
            } else {
                cen = texelFetch(splatTexture0, ivec2((splatIdx & 0x3ffu) << 1, splatIdx >> 10), 0);
            }
        ...
        uvec4 splatReadCov3d(in uint splatIdx) {
            uvec4 cov3d;
            if (bigSceneMode) {
                if (usingIndex == 0){
                    cov3d = texelFetch(splatTexture0, ivec2(((splatIdx & 0x3ffu) << 1) | 1u, splatIdx >> 10), 0);
                }else{
                    cov3d = texelFetch(splatTexture1, ivec2(((splatIdx & 0x3ffu) << 1) | 1u, splatIdx >> 10), 0);
                }
            } else {
                cov3d = texelFetch(splatTexture0, ivec2(((splatIdx & 0x3ffu) << 1) | 1u, splatIdx >> 10), 0);
            }
        ...
        vec4 splatReadColor(in uvec4 cov3d) {
            return vec4( float(cov3d.w & 0xFFu) / 255.0, float((cov3d.w >> 8) & 0xFFu) / 255.0, float((cov3d.w >> 16) & 0xFFu) / 255.0, (float(cov3d.w >> 24) / 255.0));
        }
    `;

    shaderChunk['splatWatermarkEffect'] = `
        uniform bool pointMode, showWaterMark, debugEffect;
        uniform vec4 waterMarkColor;

        vec4 getWaterMarkColor() {
            return waterMarkColor;
        }
        bool isPointMode() {
            return pointMode;
        }
        bool isWatermark(in uvec4 cen) {
            return (cen.w & 65536u) > 0u;
        }
        bool isWatermarkVisible() {
            return showWaterMark;
        }
        bool isWatermarkAnimate(in uvec4 cen) {
            return debugEffect && isWatermark(cen);
        }
    `;

    shaderChunk['splatMarkPoint'] = `
        uniform vec4 markPoint;
        bool processMarkPoint(in vec3 v3Cen, in vec4 pos2d) {
            if (markPoint.w <= 0.0 || length(vec3(markPoint.xyz) - v3Cen) > 0.000001) return false;

            vColor = vec4(1.0, 1.0, 0.0, 1.0);
            vec2 eigenVector1 = normalize(vec2(11.0, 0.0));
            vec2 eigenVector2 = vec2(eigenVector1.y, -eigenVector1.x);
            vec2 majorAxis = eigenVector1 * min(sqrt(2.0 * 11.0), 1024.0);
            vec2 minorAxis = eigenVector2 * min(sqrt(2.0 * 11.0), 1024.0);

            vPosition = vec3(position.xy, 1.0); // z提示固定不透明
    ...
    shaderChunk['splatProgressive'] = `
        uniform float topY, currentVisibleRadius, currentLightRadius;

        float getCurrentRadius (in vec3 v3Cen) {
            return length(vec3(0.0, topY, 0.0) - v3Cen);
        }
        float getCurrentLightRadius() {
            return currentLightRadius;
        }
        bool checkProgressiveVisivle(in vec3 v3Cen) {
            if ( currentVisibleRadius > 0.0 && getCurrentRadius(v3Cen) > currentVisibleRadius ) {
                gl_Position = vec4(0.0, 0.0, 2.0, 1.0);
                return false;
            }
            return true;
    ...
    shaderChunk['splatProcessWithCov3d'] = `
        void splatProcessWithCov3d(in uvec4 cen, in vec3 v3Cen, in uvec4 cov3d, in vec4 cam, in vec4 pos2d, in uint splatIndex) {
            ...
            if ( isWatermark(cen) ) {
                vColor = getWaterMarkColor();
            } else {
                if ( isPointMode() ) {
                    eigenValue1 = eigenValue2 = 0.5;
                }

                if ( !isBigSceneMode() && getCurrentLightRadius() > 0.0 ) {
                    // 仅小场景支持光圈过渡效果
                    float currentRadius = getCurrentRadius(v3Cen);
                    if ( currentRadius < getCurrentLightRadius() && currentRadius > getCurrentLightRadius() * 0.9 ) {
                        eigenValue1 = eigenValueOrig1;
                        eigenValue2 = eigenValueOrig2;
                        vColor = vec4(1.0, 1.0, 1.0, 0.2);
                    }
                    if ( currentRadius < getCurrentLightRadius() * 0.9 ){
                        if ( isPointMode() ){
                            eigenValue1 = eigenValueOrig1;
                            eigenValue2 = eigenValueOrig2;
                        } else {
            ...
            if ( vColor.a < 0.0 ) {
                vColor = splatReadColor(cov3d);
                vColor.a *= computeFvAlpha(cen);
                vColor.rgb += splatEvalSH(v3Cen, splatIndex);
            }
    ...
    shaderChunk['splatChecks'] = `
        bool checkSplatVisible(in vec4 cam, in vec4 pos2d, in uvec4 cen) {
            float clip = 1.2 * pos2d.w;
            if (pos2d.z < -clip || pos2d.x < -clip || pos2d.x > clip || pos2d.y < -clip || pos2d.y > clip
                || isWatermark(cen) && (!isWatermarkVisible() || isPointMode()) ) {
                gl_Position = vec4(0.0, 0.0, 2.0, 1.0);
                return false;
            }
            return true;
